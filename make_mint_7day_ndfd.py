# make_mint_7day_ndfd.py
# ---------------------------------------------------------
# NWS NDFD 7-Day (or max available) Minimum Temperature (°F)
# - Downloads NDFD ds.mint.bin (CONUS)
# - Reads ALL MinT messages, sorted by valid time
# - Renders up to 7 frames:
#     maps/ndfd_mint_f000.png, f001.png, ...
# - Writes:
#     maps/latest_mint_ndfd.png
#     maps/manifest_mint.json  (for slider + labels)
# - Skip logic: if valid times unchanged, skip render (unless FORCE_RENDER)
# - Local time display (EST/EDT) includes date + time
# ---------------------------------------------------------
...
NDFD_URL = (
    ...
    "AR.conus/VP.001-003/ds.mint.bin"
)

OUTDIR = "maps"
TMPFILE = os.path.join(OUTDIR, "_ndfd_mint.grib2")
STATEFILE = os.path.join(OUTDIR, "_last_ndfd_mint_7day.json")
MANIFEST = os.path.join(OUTDIR, "manifest_mint.json")
LATEST_OUTFILE = os.path.join(OUTDIR, "latest_mint_ndfd.png")
PNG_PATTERN = os.path.join(OUTDIR, "ndfd_mint_f{idx:03d}.png")
...
# Tight gradients
TEMP_MIN_F = -30   # nights can get colder than daytime
TEMP_MAX_F = 90
STEP_F = 1

# Numbers overlay
DRAW_NUMBERS = True
LABEL_DENSITY_ACROSS = 14
...
def pivotalish_temp_cmap():
    # Same palette as maxt for consistency
    anchors = [
        (-30, "#2b1055"),
        (-20, "#3f1d7a"),
        (-10, "#5b2c83"),
    ...
    return mcolors.LinearSegmentedColormap.from_list("pivotalish_temp_mint", list(zip(xs, cs)))


def read_mint_messages(grib_path: str):
    """
    Returns list of:
    ...
    try:
        msgs = grbs.select(name="Minimum temperature")
    except Exception:
        msgs = []

    if not msgs:
        try:
            msgs = grbs.select(shortName="mn2t")
        except Exception:
            msgs = []

    if not msgs:
        grbs.close()
        raise RuntimeError("No Minimum temperature (mint) messages found in GRIB")
...
def render_mint_png(data_k, lats, lons, valid_utc, outfile: str):
    data_f = k_to_f(data_k)

    # Sanity filter
    data_f = np.where((data_f < -100) | (data_f > 120), np.nan, data_f)
...
def main():
    ...
    all_msgs = read_mint_messages(TMPFILE)
    msgs = all_msgs[:MAX_FRAMES]

    if not msgs:
        raise RuntimeError("No MinT frames found after filtering")
    ...
    for idx, (data_k, lats, lons, valid_utc) in enumerate(msgs):
        ...
        render_mint_png(data_k, lats, lons, valid_utc, outfile)
